...
if ($rulers > 0){ echo "<br><div style='page-break-after: always; height:1px;'>&nbsp;</div>";
	...
	if (mt_rand(1,4) > 1)
	{
		...
		$remp_gender = "Male";
		echo "<font size='2'><b>" . strtoupper((string) $royal_title) . ":</b> <i>[Level " . GameLvlChar($game,4) . " Warrior]</i> "; 
		$equipment = GFTequipCitizen('warrior');
		$weaponsize = "medium";
		$remp_kindred = GFTRace($x_races);
		$remp_citizen = GFTCitizen($remp_kindred,'adult',$remp_gender);
		echo $remp_citizen[2] . " <b>ARMS&nbsp;&amp;&nbsp;ARMOR:</b> " . strtolower((string) getWeapon($weaponsize)) . ", " . $equipment[0] . ", " . $equipment[1] . "</font>";
			...
			echo "<hr color='#C0C0C0' size='1'><font size='2'><b>" . strtoupper((string) $royal_title) . ":</b> <i>[Level " . GameLvlChar($game,3) . " " . ucfirst($royal_class) . "]</i> "; 
