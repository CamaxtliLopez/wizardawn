...
{
	$bottom_notices = 18;
	$t_potions = ['Poison', 'Flying', 'Gaseous Form', 'Giant Strength', 'Growth', 'Healing', 'Invisibility', 'Levitation', 'Neutralize Poison', 'Remove Disease', 'Remove Paralysis', 'Shrinking', 'Stone to Flesh'];
	$bottles = count($t_potions);
	...
	<tr>
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%"><?php
			...
			if (($game == "OSRIC") || ($game == "AD&D") || ($game == "Swords & Six-Siders") || ($game == "BD&D") || ($game == "BFRPG") || ($game == "Labyrinth Lord") || ($game == "Swords & Wizardry"))
			{
				...
				<?php
						if ( strlen((string) $recipe[0]) > 2 ){array_push($mix_array, $recipe[0]);}
						if ( strlen((string) $recipe[1]) > 2 ){array_push($mix_array, $recipe[1]);}
						if ( strlen((string) $recipe[2]) > 2 ){array_push($mix_array, $recipe[2]);}
						if ( strlen((string) $recipe[3]) > 2 ){array_push($mix_array, $recipe[3]);}
						if ( strlen((string) $recipe[4]) > 2 ){array_push($mix_array, $recipe[4]);}
					$bottles = $bottles - 1;
				endwhile;
			}
			else
			{
				...
				<tr>
					<td NOWRAP style="border-left-width: 1px; border-right-width: 1px; border-top-width: 1px; border-bottom-style: solid; border-bottom-width: 1px"><font size="2"><?php echo ucwords((string) $potion[0]); ?>&nbsp;&nbsp;&nbsp;&nbsp;<br>&nbsp;-&nbsp;<?php echo $potion[2]; ?> vs. INT</font></td>
					<td style="border-left-width: 1px; border-right-width: 1px; border-top-width: 1px; border-bottom-style: solid; border-bottom-width: 1px"><font size="2"><?php echo ucfirst((string) $potion[1]) . ".&nbsp; " . potionPretty() . "&nbsp; " . $recipe[5]; ?></font></td>
				</tr>
				<?php
						if ( strlen((string) $recipe[0]) > 2 ){array_push($mix_array, $recipe[0]);}
						if ( strlen((string) $recipe[1]) > 2 ){array_push($mix_array, $recipe[1]);}
		...
		<hr>
			<table border="0" cellpadding="2" cellspacing="0" width="100%">
				<?php while ($sh_cycle > 0) :
					...
					{
						$my_mix = $mix_array[$i];
						if ($line == 1){echo "<tr><td style='border-bottom-style: solid; border-bottom-width: 1px' NOWRAP><font size='2'>" . ucwords((string) $mix_array[$i]) . ", " . ttbottlePicker() . "</font></td><td NOWRAP style='border-bottom-style: solid; border-bottom-width: 1px'  align='right' width='40'><font size='2'>" . mt_rand($value1, max((int)($value1), (int)($value2))) . "gp</font></td><td NOWRAP width='20'><font size='2'>&nbsp;</font></td>"; $line = 0;}
						else {echo "<td NOWRAP style='border-bottom-style: solid; border-bottom-width: 1px'><font size='2'>" . ucwords((string) $mix_array[$i]) . ", " . ttbottlePicker() . "</font></td><td NOWRAP style='border-bottom-style: solid; border-bottom-width: 1px' width='40' align='right'><font size='2'>" . mt_rand($value1, max((int)($value1), (int)($value2))) . "gp</font></td></tr>"; $line = 1;}
					}
					$i=$i+1;
					$sh_cycle = $sh_cycle - 1;
				endwhile;
