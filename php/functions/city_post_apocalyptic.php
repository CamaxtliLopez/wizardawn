...
else
{
		...
		$hitz = $hd;
		while ($hitz > 0) :
			$hp = $hp + mt_rand($might1, max((int)($might1), (int)($might2)));
			$hitz = $hitz - 1;
		endwhile;
}
...
//////////////////////////////////////////////////////////////////////////////////// MORALITY
if ($game == "Mutant Future"){$alignment = ['Lawful','Lawful','Lawful','Neutral','Neutral','Neutral','Neutral','Neutral','Neutral', 'Chaotic']; $aligns = "alignment";}
else {$alignment = ['Good','Good','Good','Fair','Fair','Fair','Fair','Fair','Fair', 'Evil']; $aligns = "morality";}
...
//////////////////////////////////////////////////////////////////////////////////// RACE VARIATIONS
	...
	{
		$forms = ['tree', 'cactus', 'flower', 'weed', 'fruit', 'shrub', 'vegetable', 'mossy creature', 'fungus', 'carnivorous plant'];
...
$haircolor = ['black', 'brown', 'red', 'auburn', 'gray', 'white', 'blonde'];
	...
	{
		$haircolor = ['blue-green', 'black', 'blue', 'gray', 'green', 'violet', 'red', 'brown', 'orange', 'yellowish-green', 'tan', 'gold', 'yellow', 'purple', 'silver', 'forest-green', 'white'];
...
if (($race == "Mutant Human") || ($race == "Humanoid Animal") || ($race == "Humanoid Plant") || ($race == "Humanoid Insect"))
{
	...
	$legs = ['2', '2', '2', '2', '2', '2', '2', '2', '2', '2', '2', '4', '4', '6', '6'];
		$legs = $legs[mt_rand(0,14)] . " legs";

	$arms = ['2', '2', '2', '2', '2', '2', '2', '2', '2', '2', '2', '4', '4', '4', '4'];
		$arms = $arms[mt_rand(0,14)] . " arms";

	$eyes = ['1', '2', '2', '2', '2', '2', '2', '2', '2', '2', '2', '2', '2', '3', '4', '5', '6', '7', '8'];
		$eyes = $eyes[mt_rand(0,18)];
		if ($eyes > 1){$eyec = "eyes";}
		else {$eyec = "eye";}

	$skin = ['scales', 'fur', 'feathers', 'skin']; $skin = $skin[mt_rand(0,3)];
		if ($race == "Mutant Human"){$skin = ['scales', 'fur', 'feathers', 'skin', 'skin', 'skin', 'skin', 'skin', 'skin', 'skin'];
		$skin = $skin[mt_rand(0,9)];}

	$skincolor = ['blue-green', 'black', 'blue', 'gray', 'green', 'violet', 'red', 'brown', 'orange', 'yellowish-green', 'tan', 'gold', 'yellow', 'purple', 'silver', 'forest-green', 'white'];
	...
	$eyecolor = ['blue-green', 'black', 'blue', 'gray', 'green', 'violet', 'red', 'brown', 'orange', 'yellowish-green', 'tan', 'gold', 'yellow', 'purple', 'silver', 'forest-green', 'white'];
		$eyecolor = $eyecolor[mt_rand(0,16)];
		$eyehue = ['light ', ' ', 'dark'];
		$eyehue = $eyehue[mt_rand(0,2)];
		if (($race == "Mutant Human") && (mt_rand(1,100) > 70)){$eyecolor = ['green', 'brown', 'blue']; $eyecolor = $eyecolor[mt_rand(0,2)];}
...
$interest = $interests[mt_rand(0,19)];
 if ($birth == "child"){$interest = "none";}
  if ($interest != "none")
  {
	...
	{
		$collects = ['plants and flowers', 'computer parts', 'junk', 'guns', 'antiques', 'books', 'minerals and gems', 'ornaments and jewelry', 'coins and tokens', 'trophies and skins', 'porcelain, china and crystal', 'artwork'];
...
//////////////////////////////////////////////////////////////////////////////////// EQUIPMENT
	...
	$scolor = ['gold', 'silver', 'bronze', 'copper', 'platinum', 'metal', 'iron'];
		...
		if (mt_rand(1,100) > 80)
		{
			$weapon = ['knife', 'dagger', 'ice pick', 'baton', 'club', 'crowbar', 'dart_3_ea', 'hand axe', 'razor', 'meat cleaver', 'butcher knife', 'lead pipe', 'brass knuckles', 'screwdriver', 'pipe wrench', 'slingshot_10_rocks', 'BB gun pump pistol_50_BBs', 'revolver_6_bullets', 'rusty revolver_6_bullets'];
		...
		$shirt = ['t-shirt', 'sweatshirt', 'shirt', 'hooded sweatshirt', 'long-sleeved t-shirt', 't-shirt', 'sweatshirt', 'shirt', 'hooded sweatshirt', 'long-sleeved t-shirt', 'sweater', 'tank top', 'buttoned shirt', 'golf shirt', 'jersey', 'sweater', 'tank top', 'buttoned shirt', 'dress'];
			if ($sex == "Female"){$shirtc = 18;} else {$shirtc = 17;}
			$dresses = mt_rand(0, max((int)(0), (int)($shirtc))); if ($dresses > 17){$w_legs = "";}
			$shurt = $shirt[$dresses];
				if ($shurt == "jersey")
				{
					$sides = ['front', 'back', 'front and back'];
...
{
	$hut1 = ['Tailor', 'Tailor Shop', 'Sewing Shop', 'Fine Cloth', 'Clothing Shop', 'Dress Shop', 'Dresses'];
...
{
	$hut1 = ['Medicine', 'Doctor', 'Physician', 'Hospital', 'Drug Store', 'Healer', 'Medic'];
	$hut1 = $hut1[mt_rand(0,6)];

	if (mt_rand(1,100) > 30)
	{
		$hut3 = ['Healing', 'Soothing', 'Nursing', 'Curing', 'Stitching', 'Drugged'];
...
else if ($type == 5) // STABLES ///////////////////////////////////////////////
{
	...
	if ($game == "Broken Urthe"){$hut2 = ['Beast', 'Centidon', 'Cruler', 'Ostradon', 'Steed', 'Rider', 'Charger', 'Shellox'];}
	else {$hut2 = ['Beast', 'Bull', 'Camel', 'Lion', 'Steed', 'Rider', 'Charger', 'Pony'];}
	$hut2 = $hut2[mt_rand(0,7)];
	$store = "AND store='Stable'";
}
else if ($type == 6) // VEHICLE ///////////////////////////////////////////////
{
	$hut1 = ['Engineer', 'Metal Shop', 'Wheel', 'Transporation', 'Racer', 'Traveler'];
...
{
	$hut1 = ['Pawn', 'Pawn Shop', 'Provisioner', 'Supplies', 'Equipment', 'Goods'];
	$hut1 = $hut1[mt_rand(0,5)];
...
{
	$hut1 = ['Junk', 'Scrap', 'Garbage Pickers', 'Trash Heap', 'Salvage', 'Debris', 'Lost & Found', ];
	$hut1 = $hut1[mt_rand(0,6)];
...
{
	$hut1 = ['Bank', 'Bank', 'Vault', 'Vault', 'Treasury'];
	$hut1 = $hut1[mt_rand(0,4)];

	if (mt_rand(1,100) > 30)
	{
		$hut3 = ['Wealthy', 'Guarded', 'Safe', 'Shiny', 'Rich', 'Hoarding', 'Treasured', 'Locked', 'Chained', 'Watched', 'Bolted', 'Sealed', 'Keyed'];
...
{
	$hut1 = ['Garage', 'Mechanics', 'Greasy', 'Nuts & Bolts', 'Engine Repair', 'Wooden Crafts'];
	$hut1 = $hut1[mt_rand(0,5)];
...
{
	$hut1 = ['Droid Shop', 'Mech Factory', 'Robot Repair', 'Android Market', 'Cyborg Surplus', 'Robotic Design'];
	$hut1 = $hut1[mt_rand(0,5)];

	if (mt_rand(1,100) > 30)
	{
		$hut3 = ['Iron', 'Steel', 'Rusty', 'Metallic', 'Computerized', 'Mechanical', 'Shiny', 'Haywired'];
		$hut3 = $hut3[mt_rand(0,7)];
	}
