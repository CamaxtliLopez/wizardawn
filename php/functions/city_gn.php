...
function GFTRace($races)
{
	...
	$c = $c - 1;
	$race = $my_races[mt_rand(0, max((int)(0), (int)($c)))];
...
function GFTCitizen($race,$age,$sex)
{
	...
	//////////////////////////////////////////////////////////////////////////////////// MORALITY
	$alignment = ['Good','Good','Good','Fair','Fair','Fair','Fair','Fair','Fair', 'Evil'];
	...
	{
		$haircolor = ['black', 'brown', 'red', 'auburn', 'blonde'];
	...
	else if (($race == "daklafar (dark elf)") || ($race == "dwurman (dark dwarf)") || ($race == "greyling (gray brownie)") || ($race == "suvart (deep gnome)"))
	{
		...
		else {						 $h_brd = 0;		$h_mst = 1;			$h_bld = 0;			$haircolor = ['black', 'white', 'grey'];		$hcolor = $haircolor[mt_rand(0,2)];}
	}
...
{
  $hut2 = ['Badger', 'Basilisk', 'Bear', 'Boar', 'Bufallo', 'Bugbear', 'Bull', 'Centaur', 'Chimera', 'Cloud Giant', 'Crocodile', 'Cyclops', 'Demon', 'Devil', 'Dog', 'Dragon', 'Drake', 'Dryad', 'Dwarf', 
  ...
  $hut2 = $hut2[mt_rand(0,100)];
   ...
   if (mt_rand(1,100) > 70)
   {
	$hut2 = ['Adventurer', 'Axe', 'Bandit', 'Barbarian', 'Bard', 'Baron', 'Berserker', 'Claw', 'Club', 'Coin', 'Conjurer', 'Dagger', 'Dancer', 'Druid', 'Dutchess', 'Eye', 'Fighter', 'Hammer', 'Helm', 
  ...
  $hut3 = ['Steel', 'Wooden', 'Iron', 'Bronze', 'Silver', 'Gold', 'Copper', 'Stone', 'Platinum', 'Wild', 'Sleeping', 'Lone', 'Savage', 'Scarlett', 'Twin', 'Old', 'Dark', 'Needy', 'Wealthy', 'Common', 
...
else if ($type > 16) // GUILDS /////////////////////////////////////////////////
{
	...
	if ($type == 17){$hut3 = ['Fighters', 'Warriors', 'Champions', 'Soldiers', 'Gladiators', 'Defenders', 'Mercenaries', 'Legionnaires'];}
	else if ($type == 18){$hut3 = ['Wizards', 'Mages', 'Sorcerers', 'Magicians', 'Magic-Users', 'Occultists', 'Enchanters', 'Conjurers'];}
	else {$hut3 = ['Rogues', 'Bandits', 'Burglars', 'Sneaks', 'Prowlers', 'Highwaymen', 'Muggers', 'Swindlers'];}
	...
	if ($type == 13)
	{
		...
		while ($inventory > 0) :
			if (mt_rand(1,2) == 1){	$item = explode(" (", ucwords((string) gemCreator(mt_rand(10,100))));		$price = explode(")", $item[1]);}
			else {					$item = explode(" (", ucwords((string) jewelCreator(mt_rand(10,100))));		$price = explode(")", $item[1]);}
	...
	else
	{
		...
		while ($ary=mysqli_fetch_assoc($res)) :
			...
			if ((mt_rand(1,100) <= $stock) || ($at_least_one_item != 1))
			{
				...
				$my_item = $ary['item'];
				$my_cost = $ary['cost'];
					if ($game == "Swords & Six-Siders" && (str_ends_with($ary['cost'], "cp") || str_ends_with($ary['cost'], "sp") || str_ends_with($ary['cost'], "ep")))
					{
						$my_cost = "1gp";
					}
				$add_store_item = 1;
...
function GFTequipCitizen($job) 
{
	if ($job == "rogue")
	{
		switch (mt_rand(0,1))
		{
			...
			case 1:	$shield = strtolower((string) getWeapon('small'));	break;
		}
		switch (mt_rand(0,3))
		{
